...
impl<T> IterMut<'_, T> {
    ...
    where T: std::fmt::Display
    {
        ...
        if self.head.is_null() {
            ...
            unsafe {
                let mut prev = self.head.as_mut().unwrap().prev;
                if prev.is_null() {
                    self.list.push_front(element);
                } else {
                    let node = Box::into_raw(Box::new(Node::new(element)));
                    println!("prev is {}", (*prev).element);
                    println!("head is {}", (*self.head).element);
                    node.as_mut().unwrap().next = self.head;
                    node.as_mut().unwrap().prev = prev;
                    (*prev).next = node;
                    (*self.head).prev = node;
                    self.list.len += 1;
                }
